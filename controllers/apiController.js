...
module.exports = {
    showdata: async (req, res) => {
        try {
          ...
          res.status(200).json({
            status:"Success",
            message:"get parking data Success",
            parking,
            pagination:true,
            page,
            query:{limit:queryLimit, page:queryPage}
           })
    ...
    actionAddData:async (req, res) => {
        try {
            ...
            res.status(201).json({
                status:"Success",
                message:"add parking data Success",
                parking
               })
    ...
    searchData: async (req,res)=>{
        try {
          ...
          res.status(200).json({
            status:"Success",
            message:"search parking data Success",
            parking,
            pagination:false,
    ...
    editParking:async (req, res) => {
        try {
            ...
            const { vehicle_id,parking_in,parking_out} = req.body
            var {id}= req.params
            ...
            res.status(200).json({
                status:"Success",
                message:"update parking data Success",
                parking
            })
        } catch (error) {
            res.status(500).send(`${error.message}`)
    ...
    deleteParking:async (req, res) => {
        try {
          ...
          if(parking == null){
            throw new Error("parking with id not found");
          }
      
          res.status(200).json({
            status:"Success",
            message:"delete parking data Success"
        })
        } catch (error) {
            res.status(500).send(`${error.message}`)
        }
